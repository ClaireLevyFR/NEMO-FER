...
!CT Salinity treshold {
         ! Fix to zero the salinity treshold up to 60deg North to avoid negative salinity near rivers mouth
         DO jk = 1, 4
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( gphit(ji,jj) >= 50. .AND. tsa(ji,jj,jk,jp_sal) < 0.e0 ) THEN
                      tsa(ji,jj,jk,jp_sal) = 0.1
                     IF( lwp ) THEN
                        WRITE(numout,*)
                        WRITE(numout,*) 'step - kt: ', kt, ' jk: ', jk , '  - FER NEGATIVE SSS found and limited to zero north to 50deg'
                     ENDIF
                  ENDIF
               ENDDO
            ENDDO
